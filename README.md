...
- [WZRD Velocity MCP Server](https://github.com/twzrd-sol/wzrd-final) - AI model velocity oracle MCP server with 26 tools for real-time adoption momentum across 100+ models on HuggingFace, GitHub, OpenRouter, and ArtificialAnalysis. Streamable HTTP at `app.twzrd.xyz/api/mcp`. Features model selection (`pick_model`), momentum signals, leaderboards, and agent-to-protocol interaction via Ed25519 auth. Backed by 9 Switchboard oracle feeds on Solana mainnet.
