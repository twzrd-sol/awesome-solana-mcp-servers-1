...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - Solana-native MCP server for agent trust scoring via x402 micropayments. 4 free preflight tools score any Solana wallet for trustworthiness (on-chain activity, transaction patterns, network age, recurring behavior); 4 paid tools return signed `twzrd.receipt.V6` trust tokens via HTTP 402 + USDC on Solana (<1s settlement). First Solana-native x402 trust-receipt MCP. MCP Registry: `xyz.twzrd.intel/twzrd-agent-intel`.
