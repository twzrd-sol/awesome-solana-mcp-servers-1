...
- [TWZRD Agent Intel MCP Server](https://intel.twzrd.xyz) - Trust-scoring MCP server for x402 agents on Solana. Exposes `get_trust_score` (free on-chain preflight — checks agent identity and trust signals) and `get_signed_receipt` (signed trust receipt via USDC micropayment, <$0.01, <1s settlement). Streamable-HTTP at `https://intel.twzrd.xyz/mcp`. No API key required for free tier.
...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - Solana-native MCP server for agent trust scoring via x402 micropayments. 4 free preflight tools score any Solana wallet for trustworthiness (on-chain activity, transaction patterns, network age, recurring behavior); 4 paid tools return signed `twzrd.receipt.v5` trust tokens via HTTP 402 + USDC on Solana (<1s settlement). First Solana-native x402 trust-receipt MCP. MCP Registry: `xyz.twzrd.intel/twzrd-agent-intel`.
